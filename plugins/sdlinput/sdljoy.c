...
void CheckJoy() {
	...
	for (i = 0; i < 2; i++) {
		...
		#if 0
		for(j=0;j<SDL_JoystickNumButtons(g.PadState[i].JoyDev);j++){
			if(SDL_JoystickGetButton(g.PadState[i].JoyDev, j)){
				printf("button %d\n",j);
			}
		}
		int axs=0;
		for(j=0;j<SDL_JoystickNumAxes(g.PadState[i].JoyDev);j++){
			axs = SDL_JoystickGetAxis(g.PadState[i].JoyDev, j);
				printf("%5d ",axs);			
		}
		printf("\n");
		//printf("hat %d\n", SDL_JoystickGetHat(g.PadState[i].JoyDev, 0));
		#endif
